        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <Card>
            <CardHeader className="pb-3 border-b">
              ...
              <CardDescription>
                Thiết lập các điều kiện so khớp và chấm điểm câu trả lời của AI
              </CardDescription>
            </CardHeader>
                  ...
                  <div className="space-y-4">
                    ...
                    {fieldChecksArray.fields.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-10 border border-dashed rounded-2xl bg-muted/5">
                        <ListChecksIcon className="text-muted-foreground/30 mb-2 animate-pulse" />
                        ...
                        {fieldChecksArray.fields.map((item, index) => {
                          ...
                          return (
                            <div key={item.id} className="transition-all duration-200">
                              {isEditing ? (
                                <div className="p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/30 space-y-4">
                                ...
                                <div className="p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-card hover:shadow-sm transition-all duration-200 relative group">
                                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
                                    <div className="flex flex-wrap items-center gap-2">
                                      <Badge variant="outline" className="font-mono text-xs">
                                        response.{check?.responsePath}
                                      </Badge>
                                      <span className="text-xs text-muted-foreground">đối chiếu</span>
                                      {check?.operator === 'LLM_JUDGE' ? (
                                  ...
                                  <div className="space-y-2">
                                    {check?.operator === 'LLM_JUDGE' ? (
                                      <div className="bg-zinc-50 dark:bg-zinc-900/50 border rounded-xl p-3 space-y-1.5">
                                        ...
                                        <div className="flex items-center gap-4 text-xs text-muted-foreground pt-1.5 border-t border-dashed">
                                          <span>Ngưỡng đạt: <strong>score ≥ {check?.threshold || 0.8}</strong></span>
                                        </div>
                                      </div>
                                    ) : (
                                      <div className="bg-zinc-50 dark:bg-zinc-900/50 border rounded-xl p-3">
                                        <div className="flex items-center gap-2 text-xs">
                                          <span className="font-mono">response.{check?.responsePath}</span>
                                          <span className="text-muted-foreground">
                                            {check?.operator === 'EQUALS' ? '==' : check?.operator === 'CONTAINS' ? 'contains' : check?.operator}
                                          </span>
                                          <span className={check?.expectedSource === 'DATASET_COLUMN' ? 'font-mono' : ''}>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      {llmRubricsArray.fields.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-10 border border-dashed rounded-2xl bg-muted/5">
                          <BrainCircuitIcon className="text-muted-foreground/30 mb-2" />
                        ...
                        <div className="flex flex-col gap-3">
                          {llmRubricsArray.fields.map((item, index) => (
                            <div key={item.id} className="p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-card space-y-4">
                                ...
                                <div className="flex-1">
                                  <Controller control={control} name={`llmRubrics.${index}.name`} render={({ field, fieldState }) => (
                                    <Input
                                      ...
                                      className="h-8 text-xs font-semibold w-full sm:w-[220px] rounded-lg"
