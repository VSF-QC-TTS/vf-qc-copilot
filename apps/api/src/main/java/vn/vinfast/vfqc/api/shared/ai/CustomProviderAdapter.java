...
/**
 * Custom OpenAI-compatible AI provider adapter. Supports any server implementing the OpenAI Chat
 * Completions API format (e.g. vLLM, Ollama, LM Studio, LocalAI, Text Generation Inference).
 *
    ...
    try {
      ...
      if (endpoint == null) {
        return new AiExecutionResult(
            null, 0, 0, 0, "Custom provider requires a baseUrl or endpointUrl", false);
      }
      ...
      if (request.userMessage() == null || request.userMessage().isBlank()) {
        return new AiExecutionResult(
            null, 0, 0, 0, "Custom provider userMessage is required", false);
      }
      ...
      var requestSpec = client.post().uri(endpoint).contentType(MediaType.APPLICATION_JSON);
      ...
      if (responseStr == null || responseStr.isBlank()) {
        return new AiExecutionResult(
            null, 0, 0, latency, "Empty response from Custom provider", false);
      }
      ...
      long latency = System.currentTimeMillis() - startTime;
      log.error(
          "Custom provider request failed. status={}, body={}",
          e.getStatusCode(),
