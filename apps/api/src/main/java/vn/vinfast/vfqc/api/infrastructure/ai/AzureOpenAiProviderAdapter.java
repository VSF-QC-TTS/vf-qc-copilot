...
/**
 * Azure OpenAI AI provider adapter.
 * Uses Azure-specific endpoint format with api-key header authentication.
 *
  ...
  public AiExecutionResult execute(AiRequest request) {
    ...
    try {
      if (config.getBaseUrl() == null || config.getBaseUrl().isBlank()) {
        return new AiExecutionResult(null, 0, 0, 0, "Azure OpenAI requires a baseUrl (e.g. https://{resource}.openai.azure.com)", false);
      }

      String deployment = config.resolveModel(request.useCase());
      if (deployment == null || deployment.isBlank()) {
        return new AiExecutionResult(null, 0, 0, 0, "Azure OpenAI requires a deployment name in the model field", false);
      }
      ...
      if (responseStr == null || responseStr.isBlank()) {
        return new AiExecutionResult(null, 0, 0, latency, "Empty response from Azure OpenAI", false);
      }
      ...
      long latency = System.currentTimeMillis() - startTime;
      log.error("Azure OpenAI request failed. status={}, body={}", e.getStatusCode(), e.getResponseBodyAsString(), e);
      ...
      return new AiExecutionResult(null, 0, 0, System.currentTimeMillis() - startTime, e.getMessage(), false);
    }
  }
}
