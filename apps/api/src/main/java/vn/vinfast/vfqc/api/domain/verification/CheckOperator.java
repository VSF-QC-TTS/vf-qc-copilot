...
public enum CheckOperator {
  ...
  protected static void requireExpectedValueOrColumn(FieldCheckRule rule) {
    if (rule.getExpectedSource() == ExpectedSource.LITERAL && !StringUtils.hasText(rule.getExpectedValue())) {
      throw ResourceException.of(ErrorCode.BAD_REQUEST, "expectedValue is required for LITERAL source");
    }
    if (rule.getExpectedSource() == ExpectedSource.DATASET_COLUMN && rule.getExpectedColumnKey() == null) {
      throw ResourceException.of(ErrorCode.BAD_REQUEST, "expectedColumnKey is required for DATASET_COLUMN source");
    }
  }
