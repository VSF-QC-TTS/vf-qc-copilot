...
/**
 * Google Gemini AI provider adapter. Uses the Gemini REST API with API key passed as query
 * parameter.
 *
  ...
  public AiExecutionResult execute(AiRequest request) {
    ...
    try {
      ...
      int completionTokens =
          responseJson.path("usageMetadata").path("candidatesTokenCount").asInt(0);
      ...
      if (content == null || content.isBlank()) {
        ...
        return new AiExecutionResult(
            ...
            completionTokens,
            latency,
            "Empty Gemini content. finishReason="
                + finishReason
                + ", finishMessage="
                + finishMessage,
            false);
