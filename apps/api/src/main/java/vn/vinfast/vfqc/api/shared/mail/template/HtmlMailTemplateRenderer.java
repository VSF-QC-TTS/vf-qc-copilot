package vn.vinfast.vfqc.api.shared.mail.template;
...
public class HtmlMailTemplateRenderer {
  ...
  public String render(String templatePath, Map<String, ?> model) {
    ...
    for (var entry : model.entrySet()) {
      rendered =
          rendered.replace(
              "{{" + entry.getKey() + "}}", escapeHtml(toTemplateValue(entry.getValue())));
    }

    var matcher = UNRESOLVED_PLACEHOLDER.matcher(rendered);
    if (matcher.find()) {
      throw new IllegalStateException(
          "Unresolved mail template placeholder " + matcher.group() + " in " + templatePath);
    }
    return rendered;
  }
  ...
  private String toTemplateValue(Object value) {
