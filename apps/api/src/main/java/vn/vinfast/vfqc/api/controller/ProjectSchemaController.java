...
@RestController
@RequestMapping(
    value = "/api/v1/projects/{publicId}/config/schema",
...
public class ProjectSchemaController {
  ...
  @Operation(
      summary = "Get Schema",
      description = "Gets the current project schema with all columns.")
  @GetMapping
  ...
  public ProjectSchemaResponse addColumn(
      @PathVariable UUID publicId, @Valid @RequestBody CreateSchemaColumnRequest request) {
  ...
  public ProjectSchemaResponse deleteColumn(
      @PathVariable UUID publicId, @PathVariable UUID columnPublicId) {
