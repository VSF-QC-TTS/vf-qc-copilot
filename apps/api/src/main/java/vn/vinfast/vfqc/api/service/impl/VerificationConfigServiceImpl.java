...
import vn.vinfast.vfqc.api.repository.JpaFieldCheckRuleRepository;
import vn.vinfast.vfqc.api.repository.JpaLlmRubricRuleRepository;
...
public class VerificationConfigServiceImpl implements VerificationConfigService {
  ...
  public VerificationConfigResponse get(UUID projectPublicId) {
    ...
    List<FieldCheckRule> fieldChecks =
        fieldCheckRuleRepository.findByVerificationConfigIdOrderByDisplayOrderAsc(config.getId());
    List<LlmRubricRule> llmRubrics =
        llmRubricRuleRepository.findByVerificationConfigIdOrderByDisplayOrderAsc(config.getId());
  ...
  private List<FieldCheckRule> recreateFieldChecks(
      Long configId, List<FieldCheckRuleRequest> requests) {
  ...
  private List<LlmRubricRule> recreateLlmRubrics(
      Long configId, List<LlmRubricRuleRequest> requests) {
  ...
  private void validatePreconditions(
      Long projectId, VerificationMode mode, List<FieldCheckRuleRequest> fieldChecks) {
    if (!targetConfigRepository.existsByProjectId(projectId)) {
      throw ResourceException.of(ErrorCode.MISSING_TARGET_CONFIG);
    }
    // TODO Phase 2: Switch this precondition from ProjectSchema to Dataset once Dataset import flow
    // is implemented.
    if (!schemaRepository.existsByProjectId(projectId)) {
      throw ResourceException.of(
          ErrorCode.MISSING_DATASET, "Please configure Project Schema (or upload dataset) first.");
    }

    boolean requiresAi =
        mode == VerificationMode.OVERALL_RUBRIC || mode == VerificationMode.RULE_AND_LLM;
    ...
    for (UUID key : requiredColumnKeys) {
      if (!validColumnKeys.contains(key)) {
        throw ResourceException.of(
            ...
            "Invalid expectedColumnKey for this project schema: " + key);
      }
    }
  }
