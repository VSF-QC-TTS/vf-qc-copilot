  ...
  public AiExecutionResult execute(AiRequest request) {
    ...
    try {
      ...
      if (model == null || model.isBlank()) {
        model = "claude-sonnet-4-20250514"; // Default or fallback model for Anthropic if not specified
      }
      ...
      if (content == null || content.isBlank()) {
        ...
        return new AiExecutionResult(null, 0, 0, latency, "Empty content. Stop reason: " + stopReason, false);
      }
      ...
      long latency = System.currentTimeMillis() - startTime;
      log.error("Anthropic request failed. status={}, body={}", e.getStatusCode(), e.getResponseBodyAsString(), e);
